...
Code 1: Printing the first 5 rows of our dataset.
...
Code 2: Creating a histogram for price as well as printing statistical information.
...
Code 3: Plotting a price vs. area scatter plot.

```{r, eval=FALSE}
plot(data$area, data$price, xlab='Area', ylab='Price', main='Price vs. Area', col='blue')
```

Code 4: Creating a correlation heatmap.
...
Code 5: Printing the eigenvalues and condition numbers of the model.
...
Code 6: Printing the VIF values for each predictor.
...
Code 7: Printing the $R^2_k$ values for each predictor.

```{r, eval=FALSE}
1 - 1/vif(model)
```

Code 8: Printing the eigenvalues and condition numbers for each predictor.

```{r, eval=FALSE}
round(ols_eigen_cindex(model), 3)
```

Code 9: Performing forward, backward, and step-wise model selection.
...
Code 10: Printing the LOO-CV RMSE and Adjusted $R^2$ for each model.
...
Code 11: Performing a Breusch-Pagan test.
...
Code 12: Performing a Shapiro-Wilk test.

```{r, eval=FALSE}
shapiro.test(resid(model))
```

Code 13: Creating a Box Cox model and printing the maximum.
...
Code 14: Performing a Shapiro-Wilk test.
...
Code 15: Printing the $R^2$ values and RMSE of each model.
...
Code 16: Printing how many observations are influential.

```{r, eval=FALSE}
length(which(cooks.distance(model_bc) > 4 / length(cooks.distance(model_bc))))
```

Code 17: Performing a Breusch-Pagan test on the model without influential points.
...
Code 18: Performing a Shapiro-Wilk test on the model without influential points.

```{r, eval=FALSE}
shapiro.test(resid(model_fix))
```

Code 19: Printing the $R^2$ value of each model.
...
Code 20: Printing out the number of observations deemed to be outliers.
...
Code 21: Performing WLS regression.
...
Code 22: Printing the coefficients of the OLS and WLS regressions.
...
Code 23: Printing the $R^2$ and RMSE of the OLS and WLS regressions.
...
Code 24: Printing the final coefficients.

```{r, eval=FALSE}
summary(model_wls)$coef
```

Code 25: Performing variable selection again on our and refined model.
